with
    transactions_info as (
        select
            ...
            AVG(amount_usd) as avg_amount_usd_completed
        from `neobank.transactions_devices`
        where transactions_state = 'COMPLETED'
        group by 1
    ), notifications_info as (
        select user_id, 
            COUNT(*) as notif_count 
        from `neobank.notifications`
        group by user_id
...
select
    ...
    t.first_action_date,
    t.last_action_date,
    date_diff(t.first_action_date, date(u.created_date), day) as days_sub_first,
    date_diff(t.last_action_date, date(u.created_date), day) as days_sub_last,
...
left join notifications_info noti on noti.user_id = u.user_id